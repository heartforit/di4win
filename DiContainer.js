...
class DiContainer extends node_events_1.default {
    get containerInitialised() {
        return this._containerInitialised;
    }
    constructor(options) {
        super();
        this._context = new Map;
        this._setupScripts = new Map;
        this._diDependencies = new Map();
        this._store = {};
    ...
    emitLog(level, message) {
        this.emit("onLog", { level, message, callstack: new Error().stack });
    }
    async init(config) {
        if (this._containerInitialised)
            throw new Error("container is already initialised");
        if (config.store) {
            if (typeof config.store !== "object")
                throw new Error("store must be type of object");
        ...
        if (config.items) {
            for (const _item of config.items) {
                ...
                if (_item.setup) {
                    ...
                    const _resolvedPath = node_path_1.default.resolve(_fileToImport);
                    const _import = await Promise.resolve(`${_resolvedPath}`).then(s => __importStar(require(s)));
                    if (_namedImport === "*") {
        ...
        if (this._options.emitDependencies) {
            const diDependenciesWithoutConfig = new Map();
            for (const _item of this._diDependencies.values()) {
                if (!idsFromConfig.has(_item.id)) {
                    diDependenciesWithoutConfig.set(_item.id, _item);
                }
            }
            this.emit("dependenciesResolved", this._diDependencies.values());
            this.emit("dependenciesResolvedWithoutConfig", diDependenciesWithoutConfig);
        }
    ...
    async containerStartedHandleEvents() {
        if (this._containerInitialised)
            throw new Error("container is already initialised");
        ...
        this.emit("onContainerReady", this);
    }
    async shutdown() {
        if (!this._containerInitialised)
        ...
        this.emit("onContainerShutdown", this);
        this._context.clear();
        this._containerInitialised = false;
    }
    async resolveDependencyClasses(_searchPaths) {
    ...
    async createInstanceOf(func, args) {
